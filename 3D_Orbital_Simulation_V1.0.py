...
x_E = R_Earth * np.sin(v) * np.cos(u)                 # Spherical Coordinate formula        
y_E = R_Earth * np.sin(v) * np.sin(u)
z_E = R_Earth * np.cos(v)
ax.plot_surface(x_E, y_E, z_E, color = 'orange', alpha = 0.7, label = "Earth")
...
def update(frame):
    ...
    x_S, y_S, z_S = positions[frame]
    Sat_Mod._offsets3d = ([x_S], [y_S], [z_S])
    return
